...
class ControlModule:
    ...
    @staticmethod
    def control_iteration(P,R, state, gamma)-> np.int32:
    
        """ Function that computes one control-iteration """
        ### TO BE COMPLETED BY THE STUDENTS ###
        sol = mdptoolbox.mdp.ValueIteration(P,R,gamma)
        sol.run()
        policy = np.array(sol.policy) 
    ...
    def control_loop(demand, probs, n_states, n_actions, gamma) -> np.ndarray:
        ...
        res = np.zeros_like(a=demand, dtype=np.float64)    
        #initial state 
        curr_st = np.int32(round(demand[0] * (n_states - 1))) 
        move = {0:[-2,-1,0], 1:[-1,0,1],2:[0,1,2]} 
        for t in range(demand.shape[0]):
            ...
            sel_move = np.random.choice(
                move[int(action)],
                p=probs[int(action)]
                ) 
            curr_st = np.clip(curr_st + sel_move, 0, n_states - 1)
            res[t] = curr_st / (n_states - 1) #normalizing 
        return res 
